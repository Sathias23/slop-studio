...
5. Call `check_next_job` with the prompt_id in a list and `wait: 30` to poll for completion
6. If the job is still in `remaining`, call `check_next_job` again with `wait: 30`
7. Once the job appears in `completed`, call `get_image` to get the output file path
